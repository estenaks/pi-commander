...
class LCD_3inch5(framebuf.FrameBuffer):
    def __init__(self):
        ...
        self._buf1 = bytearray(1)   # pre-alloc for _cmd/_dat
        self._init()
...
# Pre-compute colour wheel — 12 segments, fixed, no per-frame allocation
_WHEEL = []
for _s in range(12):
    _h = int((_s / 12) * 360)
    _hi = (_h // 60) % 6
...
# Pre-compute scroll message as fixed-length buffer label
_SCROLL_MSG = "  pi-commander  320x480  portrait  "
_SCROLL_LEN = len(_SCROLL_MSG) * 8   # pixels

# Reusable frame label buffer
...
import gc
gc.collect()   # clean up after static sections before animation loop
...
while True:
    ...
    lcd.show_down()
    gc.collect()   # collect every frame — keeps heap flat
    frame += 1
    time.sleep_ms(50)